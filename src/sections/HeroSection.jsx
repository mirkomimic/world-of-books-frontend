...
const HeroSection = () => {
  ...
  return (
    // xywh(0 10% 100% 75% round 15% 0)
    <section className="flex flex-col justify-around h-[calc(100vh-80px)] items-center">
      <motion.img
        src="/open-book.jpg"
        alt="open-book"
        className="w-full md:w-10/12 h-full absolute -z-10 top-0"
      ...
      <motion.div
        className="w-full md:w-10/12 h-full absolute -z-9 top-0 bg-gradient-to-r from-yellow-950/70 to-gray-300/40"
        style={{ clipPath: `xywh(0 10% 100% 75% round 15% 0)` }}
        initial={{ clipPath: `inset(10% 0% 15% 0%)` }}
        animate={{ clipPath: `inset(10% 0% 15% 0% round 15% 0)` }}
        transition={{ duration: 1, delay: 2 }}
      />
      ...
      >
        {[...Array(3)].map((_, a) => (
          <MotionCard key={a} variants={cardsVariants} />
        ))}
      </motion.div>
    </section>
  );
...
const HeroCard = ({ ref }) => {
  return (
    <div
      ref={ref}
      className="backdrop-blur-lg bg-gray-400/10 h-64 w-80  rounded-lg text-center pt-3 flex flex-col"
    >
